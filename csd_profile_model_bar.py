...
def gauss_2d_large(x, y, seed=0, N = 1):
...
def draw_line(im, point_num = 50):
    lines = []
    plt.imshow(im, extent = [0,1,0,1], alpha = 0.4)
    ax = plt.gca()
    xy = plt.ginput(point_num)
    plt.xlim(0,1)
    plt.ylim(0,1)
    x = [p[0] for p in xy]
    y = [p[1] for p in xy]
...
def gauss_2d_small(x, y, src_width, seed=0, N=1, tpy='dip', srcs=np.array([0])):
...
#    states = get_states_2D(seed)
    ...
    for j in range(N):
        y_shift = np.linspace(0.1,0.6, N)
#            f0 = gauss2d(x,y,[0.5,y_shift[j],x_amp,y_amp,0.5,angle])
        f1 = gauss2d(x,y,[0.5,y_shift[j]+0.2,x_amp,y_amp,0.5,angle])
        f2 = gauss2d(x,y,[0.5,y_shift[j]+0.4,x_amp,y_amp,0.5,angle])
        f3 = gauss2d(x,y,[0.5,y_shift[j]+0.6,x_amp,y_amp,0.5,angle])
    if tpy == 'two_sink':
        f[:,:] = 1*f1-0.5*f2-0.5*f3
    elif tpy == 'quadr':
        f[:,:] = -0.5*f1+1*f2-0.5*f3
    elif tpy == 'dip':
        f[:,:] = 1*f1-1*f2
    elif tpy == 'ob.png':
#        df = pd.read_excel('97_hist.xlsx', skiprows = 0, index_col = None) #obcord
        df = pd.read_excel('ob.xlsx', skiprows = 0, index_col = None) #obcord
#        logo = img_as_float(io.imread('ob.png'))
#        xp, yp = draw_line(logo, points) # here you click on the plot
        ...
        sig  = 0.04
        amp = src_width
        for sgn in range(points): 
            f+= ((-1)**sgn)*gauss2d(x, y, [xpos[sgn], ypos[sgn], sig,  sig, amp, angle]) 
    elif tpy == 'hipo':
        df = pd.read_excel('hipo.xlsx', skiprows = 0, index_col = None) #obcord
        xpos = df['x pos'].values
        ypos = df['y pos'].values
        points = len(xpos)
        f = np.zeros((len(x),len(y)))
